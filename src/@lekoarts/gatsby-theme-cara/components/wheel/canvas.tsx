...
export default function WheelCanvas() {
  ...
  const [isP2Loaded, setP2Loaded] = useState(false);
  ...
  var statusLabel = statusLabelRef.current;

  useEffect(() => {
    if (!drawingCanvasRef.current || !statusLabelRef.current || !isP2Loaded)
      return;
  ...
  }, [drawingCanvasRef.current, statusLabelRef.current, isP2Loaded]);
  ...
  function checkStartDrag(e) {
    ...
    if (wheelSpinning === true) {
      ...
      wheelStopped = true;
      if (statusLabel)
        statusLabel.innerText = 'Impatience will not be rewarded.';
    }
  }
  ...
  function checkEndDrag(e) {
    if (mouseConstraint) {
      ...
      if (wheelSpinning === false && wheelStopped === true) {
        if (Math.abs(wheel.body.angularVelocity) > 5.0) {
          wheelSpinning = true;
          wheelStopped = false;
          if (statusLabel) statusLabel.innerText = '';
        } else {
          if (statusLabel)
            statusLabel.innerText = 'You can spin harder than that!';
        }
      }
    }
  }
  ...
  function initPhysics() {
    ...
    wheel = new Wheel(wheelX, wheelY, wheelRadius, tips.length, 0.25, 3.5);
  ...
  function update() {
    ...
    if (
      ...
      //spawnParticles();
      if (statusLabel) statusLabel.innerText = tips[win].tip;
    }
  }
  ...
  Wheel.prototype = {
    ...
    gotLucky: function () {
      ...
      return Math.abs(currentSegment) - 1;
    },
    draw: function () {
      ...
      ctx.beginPath();
      ctx.fillStyle = '#eee';
      ctx.arc(0, 0, this.pRadius + 1, 0, TWO_PI);
      ctx.fill();
      ...
      for (var i = 0; i < this.segments; i++) {
        ctx.fillStyle = colors[tips[i].type] ?? '#FFFAD5';
        ctx.beginPath();
  ...
  return (
    <div className="wheel-outer">
      <div className="wheel-container">
        <div className="status-label" ref={statusLabelRef}>
          Spin the wheel to get a free tip!
        </div>
        <canvas ref={drawingCanvasRef}></canvas>
      </div>
      <Script
        onLoad={() => setP2Loaded(true)}
        src="//cdnjs.cloudflare.com/ajax/libs/p2.js/0.6.0/p2.min.js"
      />
    </div>
  );
}
...
const tips = [
  {
    type: 'Design',
    tip: "Design tip: put yourself in your user's shows",
  },
  {
    type: 'Growth',
    tip: 'Growth tip: know your audience and their habits',
  },
  {
    type: 'Mobile',
    tip: 'Mobile app tip: choose the right technology',
  },
  { type: 'SEO', tip: 'SEO tip: focus on creating good, relevant content' },
  { type: 'Design', tip: 'Design tip: use empty states to your advantage' },
  {
    type: 'Growth',
    tip: 'Growth tip: become a thought leader in your field',
  },
  {
    type: 'Mobile',
    tip: 'Mobile app tip: prioritize the safety of your users and your data',
  },
  { type: 'SEO', tip: 'SEO tip: define good metrics and measure them' },
  {
    type: 'Design',
    tip:
      'Design tip: use negative space to add breathing room to your user interfaces',
  },
  {
    type: 'Growth',
    tip: 'Growth tip: focus on building a truly useful product',
  },
  {
    type: 'Mobile',
    tip: 'Mobile app tip: test on a wide variety of devices',
  },
  {
    type: 'SEO',
    tip: 'SEO tip: use a descriptive headline high on the homepage',
  },
];
